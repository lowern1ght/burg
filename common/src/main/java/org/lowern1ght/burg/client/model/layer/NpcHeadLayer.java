...
import org.lowern1ght.burg.client.model.NpcHeadModels;
...
public class NpcHeadLayer<T extends Mob, M extends NpcModel<T>> extends RenderLayer<T, M> {
     ...
     * "No model for layer", which failed the whole {@code burg:npc} model, which killed
    ...
    public void render(PoseStack pose, MultiBufferSource buffer, int packedLight, T npc,
        ...
        // `burg:npc` model takes the entity renderer with it, and the game then runs
