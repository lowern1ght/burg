...
public class Road extends SliceBuild {
    ...
    private void tryExtendLength(ProtoTown town) {
        if (this.canGrow) {
            ...
            int bonusSize = canGrow ? DEFAULT_PATH_LENGTH : 0;
     ...
     */
    private void extendLength(int extendInDir, int extendInOppositeDir) {
        length += extendInDir + extendInOppositeDir;
        // We move the YShape so that the locked shapes stay at the same position.
        SliceProperty[] newYShape = new SliceProperty[length];
        System.arraycopy(yShape, 0, newYShape, extendInOppositeDir, yShape.length);
        yShape = newYShape;
    }
